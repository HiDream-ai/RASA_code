...
from ..models.utils import load_state_dict, custom_load_state_dict_from_safetensors
...
class DiffusionTrainingModule(torch.nn.Module):
    ...
    def export_trainable_state_dict(self, state_dict, remove_prefix=None):
        ...
        if remove_prefix is not None:
            all_remove_prefix = remove_prefix.split(",")
            state_dict_ = {}
            for name, param in state_dict.items():
                for remove_prefix in all_remove_prefix:
    ...
    def switch_pipe_to_training_mode(
        ...
        # lora_path = "models/train/merge_cim/epoch-11.safetensors"
        ...
        if lora_base_model is not None:
            ...
            if lora_checkpoint is not None:
                ...
                custom_dict = custom_load_state_dict_from_safetensors(lora_checkpoint)
                pipe.convert.load_state_dict(custom_dict, strict=True)
...
def wan_parser():
    ...
    parser.add_argument("--num_frames", type=int, default=41, help="Number of frames per video. Frames are sampled from the video prefix.")
    parser.add_argument("--data_file_keys", type=str, default="image,video,pose", help="Data file keys in the metadata. Comma-separated.")
    ...
    parser.add_argument("--onetomany", default=1, type=int, help="Whether to train with one-to-many training strategy. If set to 1, one image will be paired with multiple videos during training, which can improve the training efficiency when the dataset contains many videos but few images.")
    return parser
