...
from ..models import ModelManager, load_state_dict, custom_load_state_dict_from_safetensors, custom_load_state_dict_from_safetensors_vace
...
# from ..models.wan_video_custom import WanCustom
# from ..models.wan_video_custom_v2 import WanCustom
from ..models.wan_video_custom_v3 import WanCustom
from ..schedulers.flow_match import FlowMatchScheduler
from ..prompters import WanPrompter
from ..vram_management import enable_vram_management, AutoWrappedModule, AutoWrappedLinear, WanAutoCastLayerNorm
from ..lora import GeneralLoRALoader
from ..models.encdec import Encoder, encoder_state_dict
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.convert: WanCustom = None
        self.motion_encoder: Encoder = None
        self.vae: WanVideoVAE = None
        self.motion_controller: WanMotionControllerModel = None
        self.vace: VaceWanModel = None
        self.vace2: VaceWanModel = None
        self.in_iteration_models = ("dit", "motion_controller", "vace", "convert")
        ...
        self.units = [
            ...
            # WanVideoUnit_S2V(),
            WanVideoUnit_InputPoseEmbedder(), ###add
            WanVideoUnit_MotionLatent(),
        ...
        self.model_fn = bind_patch_model_fn_wan_video  #patch_model_fn_wan_video  ##new_patch_model_fn_wan_video 
    ...
    def load_trainable(self, module, path):
        custom_lora = custom_load_state_dict_from_safetensors(path, torch_dtype=self.torch_dtype, device=self.device)
        module.load_state_dict(custom_lora)
        
    def load_trainable_vace(self, module, path):
        custom_lora = custom_load_state_dict_from_safetensors_vace(path, torch_dtype=self.torch_dtype, device=self.device)
        module.load_state_dict(custom_lora)
    ...
    def from_pretrained(
        ...
        pipe.convert = WanCustom(in_channels=3072, out_channels=1536).to(torch.bfloat16) ###add
        pipe.motion_encoder = Encoder().to(torch.bfloat16)
        pipe.motion_encoder.load_state_dict(encoder_state_dict, strict=True)
    ...
    def __call__(
        ...
        # Prompt
        prompt: str,
        motion_sequence: Optional[np.array] = None,
        driven_pose: Optional[list[Image.Image]] = None,
        ...
        # Video-to-video
        input_video_clip: Optional[list[Image.Image]] = None,
        ...
        inputs_shared = {
            ...
            "end_image": end_image, "input_video_clip": input_video_clip,
            "input_video": input_video, "denoising_strength": denoising_strength,
            "motion_sequence": motion_sequence, "driven_pose": driven_pose,
...
# class WanVideoUnit_MotionLatent(PipelineUnit):
#     def __init__(self):
#         super().__init__(
#             input_params=("motion_sequence",))
#     def process(self, pipe: WanVideoPipeline, motion_sequence):
#         if isinstance(motion_sequence, np.ndarray):
#             motion_sequence = torch.tensor(motion_sequence).to(dtype=pipe.torch_dtype, device=pipe.device)
#             return {"motion_latents": motion_sequence}
#         else:
#             return {}
...
class WanVideoUnit_MotionLatent(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, motion_sequence):
        if isinstance(motion_sequence, np.ndarray):
            motion_sequence = torch.tensor(motion_sequence).to(dtype=pipe.torch_dtype, device=pipe.device)
            pipe.motion_encoder.to(device=pipe.device)
            # motion_sequence[:, :, -4:] = torch.round(torch.clamp(motion_sequence[:, :, -4:], 0, 1))
            # motion_sequence = torch.cat([motion_sequence[:,:1].repeat(1, 3, 1), motion_sequence[:,:41]], 1)
            motion_sequence = pipe.motion_encoder(motion_sequence).permute(0,2,1)
            # motion_sequence = motion_sequence.to(dtype=pipe.torch_dtype)
            return {"motion_latents": motion_sequence}
        else:
            return {}
 
        
class WanVideoUnit_InputPoseEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("driven_pose", "noise", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, driven_pose, noise, tiled, tile_size, tile_stride):
        if driven_pose is None:
            return {}
        pipe.load_models_to_device(["vae"])
        driven_pose = pipe.preprocess_video(driven_pose)
        pose_latents = pipe.vae.encode(driven_pose, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
        return {"driven_pose": pose_latents}
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        if input_video is None and vace_reference_image is not None:
            if not isinstance(vace_reference_image, list):
                vace_reference_image = [vace_reference_image]
            vace_reference_image = pipe.preprocess_video(vace_reference_image)
            vace_reference_latents = pipe.vae.encode(vace_reference_image, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
            # vace_reference_latents = pipe.scheduler.add_noise(vace_reference_latents, noise[:,:,:1], timestep=pipe.scheduler.timesteps[15])
            # noise[:,:,:1] = vace_reference_latents
            return {"latents": noise, "vace_reference_latents":vace_reference_latents}
        ...
        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents, "vace_reference_latents":vace_reference_latents}
        else:
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video_clip", "input_video","end_image", "height", "width"),
            onload_model_names=("image_encoder",)
        )

    def process(self, pipe: WanVideoPipeline, input_video_clip, input_video, end_image, height, width):
        if input_video is not None:
            input_video_clip = input_video

        if input_video_clip is None or pipe.image_encoder is None or not pipe.dit.require_clip_embedding:
            return {}
        if input_video is not None:
            input_video_clip = input_video
        pipe.load_models_to_device(self.onload_model_names)
        # image = pipe.preprocess_image(input_video[0].resize((width, height))).to(pipe.device)
        # clip_context = pipe.image_encoder.encode_image([image])
        input_video = pipe.preprocess_video(input_video_clip).permute(0,2,1,3,4)
        clip_context = pipe.image_encoder.encode_image(input_video)
        clip_context = clip_context.to(dtype=pipe.torch_dtype, device=pipe.device)
        return {"clip_feature": clip_context}  # only use the first frame's embedding
...
def bind_patch_model_fn_wan_video(
    dit: WanModel,
    convert: WanCustom,
    ...
    vace_context = None,
    vace_scale = 1.0,
    vace_reference_latents: torch.Tensor = None,
    audio_embeds: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    latents = torch.concat([vace_reference_latents, latents], dim=2)
    
    x = latents

    if driven_pose is not None:
        patch_pose = convert.patchify(driven_pose)
    # Add camera control
    ...
    if motion_latents is not None:
        use_motion_latents = True
        motion_latents = motion_latents[:,:,:, None, None]
        motion_latents = rearrange(motion_latents, 'b f c h w -> b f h w c').contiguous()
        motion_latents = convert.motion_proj(motion_latents)
        motion_latents = convert.norm_out(motion_latents)
        motion_latents = torch.concat([motion_cond_proj(motion_latents) for motion_cond_proj in convert.motion_cond_projs], 0)
        ...
        motion_latents = motion_latents.reshape(x.shape[0], motion_latents.shape[0] // x.shape[0], -1, *motion_latents.shape[2:])  # b, f, c, h, w)
    else:
        use_motion_latents = False
    ...
    if tea_cache_update:
        ...
        for block_id, block in enumerate(dit.blocks):
            if use_motion_latents:
                mo_idx = None
                if (block_id <= len(dit.blocks) // 2 and block_id > 1):
                    mo_idx = block_id - 2
                    # motion_emb_tmp = motion_latents[:, mo_idx].repeat(1, 1, h , w , 1) 
                    # motino_bind = motion_latents[:, mo_idx].permute(0, 4, 1, 2, 3)
                    # motino_bind = convert.attn(motion_latents[:, mo_idx].permute(0, 4, 1, 2, 3),patch_pose)
                    motino_bind = convert.attn(motion_latents[:, mo_idx].permute(0, 4, 1, 2, 3),patch_pose,clip_feature.unsqueeze(0))
                    motino_bind = motino_bind.repeat(1, 1, 1, h, w) 
                    motion_emb_tmp = convert.motion_patchify(motino_bind)
                    # motion_emb_tmp = convert.motion_patchify(motion_emb_tmp.permute(0, 4, 1, 2, 3))
                    
                    x = x + motion_emb_tmp * 1.0
    ...
    if vace_reference_latents is not None:
        x = x[:, :, vace_reference_latents.shape[2]:]
    return x

def new_patch_model_fn_wan_video(
    dit: WanModel,
    convert: WanCustom,
    motion_controller: WanMotionControllerModel = None,
    vace: VaceWanModel = None,
    ...
    **kwargs,
    ...
    vace_reference_latents, _ = dit.patchify(vace_reference_latents, None)
    vace_reference_latents = vace_reference_latents.repeat(1, 1, latents.shape[2], 1, 1)
    latents_concatenated = torch.cat([x, vace_reference_latents], dim=1)
    x = convert.ref_patchify(latents_concatenated)
    x+=patch_pose
        ...
        for block_id, block in enumerate(dit.blocks):
            if use_motion_latents:
                ...
                if (block_id <= len(dit.blocks) // 2 and block_id > 1):
                    mo_idx = block_id - 2
                    motion_emb_tmp = motion_latents[:, mo_idx].repeat(1, 1, h , w , 1)
                    # motino_bind = convert.attn(motion_emb_tmp.permute(0, 4, 1, 2, 3),patch_pose)
                    # motion_emb_tmp = convert.motion_patchify(motino_bind)

                    motion_emb_tmp = convert.motion_patchify(motion_emb_tmp.permute(0, 4, 1, 2, 3))
                    
                    x = x + motion_emb_tmp * 0.6
            if use_gradient_checkpointing_offload:
                with torch.autograd.graph.save_on_cpu():
                    x = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(block),
...
def patch_model_fn_wan_video(
    ...
    **kwargs,
    ...
    # latents = torch.concat([vace_reference_latents, latents], dim=2)
    latents = torch.concat([vace_reference_latents, latents+vace_reference_latents*0.5], dim=2)
