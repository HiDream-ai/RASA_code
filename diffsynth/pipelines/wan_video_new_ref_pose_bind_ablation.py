...
from ..models.wan_video_custom_retarget_bind import WanCustom
...
class WanVideoUnit_MotionLatent(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, motion_sequence):
        if isinstance(motion_sequence, np.ndarray):
            ...
            # flops = FlopCountAnalysis(pipe.motion_encoder, motion_sequence)
            # print("FLOPs:", flops.total() / 1e9, "GFLOPs")

            # print(parameter_count_table(pipe.motion_encoder))
...
class WanVideoUnit_InputPoseEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, driven_pose, noise, tiled, tile_size, tile_stride):
        if driven_pose is None:
            return {}
        ref_pose = driven_pose[1]
        pipe.load_models_to_device(["vae"])
        driven_pose = pipe.preprocess_video(driven_pose[0])
        ...
        ref_pose = pipe.preprocess_video(ref_pose)
        ref_pose_latents = pipe.vae.encode(ref_pose, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)

        return {"driven_pose": pose_latents, "ref_pose": ref_pose_latents}
...
def bind_patch_model_fn_wan_video(
    ...
    **kwargs,
    ...
    if driven_pose is not None:
        ...
        ref_pose = convert.patchify(ref_pose)
    # Add camera control
    x, (f, h, w) = dit.patchify(x, control_camera_latents_input)
    
    # x[:, :, 1:] += patch_pose
    # freqs = dit.freqs(use_motion=True).to(x.device)
    freqs = torch.cat([
    ...
    align_pose = convert.align_model(ref_pose, patch_pose)
    
    x[:, :, 1:] += align_pose
    align_pose = rearrange(align_pose, 'b c f h w -> b (f h w) c').contiguous()
